...
export class AppointmentListComponent implements OnInit, OnDestroy {
  ...
  protected get confirmed() {
    return this.appointments().filter((a) => a.status === 'CONFIRMADA').length;
  }
  protected get pending() {
    return this.appointments().filter((a) => a.status === 'PENDIENTE').length;
  }
  protected get completed() {
    return this.appointments().filter((a) => a.status === 'COMPLETADA').length;
  }
  ...
  private loadProfessionals(): void {
    const sub = this.professionalSvc.getAll().subscribe({
      next: (data) => {
        this.professionals.set(data.filter((p) => p.isActive));
      },
    });
    this.subs.add(sub);
  }
