...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  protected get showAgenda() {
    return this.auth.hasRole('ADMINISTRADOR', 'AGENDADOR', 'MEDICO', 'TERAPISTA');
  }
  protected get showQuickActions() {
    return this.auth.hasRole('AGENDADOR');
  }
  protected get showProfessionals() {
    return this.auth.hasRole('ADMINISTRADOR');
  }
  protected get showPatientBooking() {
    return this.auth.hasRole('PACIENTE');
  }
  protected get showAppointmentMetrics() {
    return this.auth.hasRole('ADMINISTRADOR', 'AGENDADOR', 'MEDICO', 'TERAPISTA');
  }
  protected get showPatientMetrics() {
    return this.auth.hasRole('PACIENTE');
  }
  ...
  protected get filteredAppointments(): Appointment[] {
    return this.appointments().filter((a) => {
      ...
      const byProf =
        !this.selectedProfessionalId || a.professional.id === this.selectedProfessionalId;
  ...
  protected get totalToday() {
    return this.filteredAppointments.length;
  }
  protected get totalConfirmed() {
    return this.filteredAppointments.filter((a) => a.status === 'CONFIRMADA').length;
  }
  protected get totalPending() {
    return this.filteredAppointments.filter((a) => a.status === 'PENDIENTE').length;
  }
  protected get totalCompleted() {
    return this.filteredAppointments.filter((a) => a.status === 'COMPLETADA').length;
  }
